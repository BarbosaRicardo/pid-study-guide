...
import { X, Download, Trash2, RefreshCw, CheckCircle2, XCircle, BarChart2 } from 'lucide-react'
...
function exportCSV(submissions) {
  ...
  const rows = [...submissions].reverse().map(s =>
    [fmtDate(s.ts), chapterLabel(s.chapter), `Level ${s.level}`, s.score, s.total, `${s.pct}%`, s.passed ? 'Pass' : 'Fail', s.attempt].join(',')
  )
  const blob = new Blob([[header, ...rows].join('\n')], { type: 'text/csv' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `quiz_report_${Date.now()}.csv`
  a.click()
  URL.revokeObjectURL(url)
}

export default function QuizReport({ onClose }) {
  const { submissions, refresh, clearAll } = useQuizReport()
  ...
  const passed = sorted.filter(s => s.passed).length
  const avgPct = total ? Math.round(sorted.reduce((a, s) => a + s.pct, 0) / total) : 0
  const byLevel = [1, 2, 3].map(l => ({ level: l, count: sorted.filter(s => s.level === l).length }))
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 flex-shrink-0"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500">{total} submission{total !== 1 ? 's' : ''} · stored locally</div>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-white transition-colors"
        ...
        {total > 0 && (
          <div className="flex gap-4 px-5 py-2.5 flex-shrink-0"
            ...
            <button onClick={refresh} className="ml-auto text-slate-600 hover:text-slate-400 transition-colors">
              <RefreshCw size={13} />
            </button>
          </div>
        )}
              ...
              <tbody>
                {sorted.map((s, i) => (
                  <tr key={s.id} style={{
                    ...
                    <td className="px-4 py-2.5 text-slate-500 whitespace-nowrap">{fmtDate(s.ts)}</td>
        ...
        <div className="flex items-center gap-3 px-5 py-3 flex-shrink-0"
          ...
          {confirmClear && (
            <button onClick={() => setConfirmClear(false)} className="text-xs text-slate-600 hover:text-slate-400">
              Cancel
            </button>
          )}
          <span className="ml-auto text-xs text-slate-700">Data stored in browser only</span>
        </div>
      </div>
    </>
  )
